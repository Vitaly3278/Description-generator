...
const FREE_LIMIT = 5; // бесплатных коротких генераций

const STYLES = [
  { id: 'short', label: 'Краткое', desc: '2-3 предложения', free: true },
  { id: 'standard', label: 'Стандартное', desc: 'Полное описание', free: false },
  { id: 'seo', label: 'SEO', desc: 'С ключевыми словами', free: false },
];
...
function MainPage() {
  ...
  const [freeUsed, setFreeUsed] = useState(() => {
    try { return parseInt(localStorage.getItem('free-gens') || '0', 10); } catch { return 0; }
  });
  ...
  useEffect(() => {
    if (history.length > 0) {
      try { localStorage.setItem('description-history', JSON.stringify(history.slice(0, 50))); } catch (e) {}
    }
  }, [history]);
  ...
  const generateDescription = async () => {
    ...
    // Без авторизации — проверка лимита бесплатных
    ...
    const promise = new Promise((resolve, reject) => {
      xhr.addEventListener('load', () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          reject(new Error('Для этого стиля нужна авторизация'));
        } else if (xhr.status === 402) {
          reject(new Error(`Недостаточно средств. Баланс: ${xhr.getResponseHeader('X-Balance') || '0'}₽`));
        } else {
          try {
    ...
    try {
      ...
      // Увеличиваем счётчик бесплатных
      if (!token) {
        const newUsed = freeUsed + 1;
        setFreeUsed(newUsed);
        localStorage.setItem('free-gens', String(newUsed));
      }

      const firstLine = data.description.split('\n')[0]?.replace(/[#*_]/g, '').trim() || 'Товар';
      const item = {
        id: Date.now(),
        description: data.description,
        style: selectedStyle,
        productName: firstLine.slice(0, 30),
  ...
  const clearHistory = () => { setHistory([]); localStorage.removeItem('description-history'); };

  const freeRemaining = FREE_LIMIT - freeUsed;
    ...
    <div className={`App ${darkMode ? 'dark' : ''}`}>
      ...
      <div className="top-bar">
        <div className="top-bar-content">
          ...
          <div className="top-bar-right">
            {!user ? (
              <>
                <div className="free-counter" title={`Осталось бесплатных: ${freeRemaining}`}>
                  <Sparkles size={14} />
                  <span>{freeRemaining} бесплатных</span>
                </div>
      ...
      <header className="hero">
        ...
        <div className="container hero-content">
          {!user && <div className="hero-badge">🚀 5 бесплатных генераций без регистрации</div>}
          {user && <div className="hero-badge">👋 Привет, {user?.email?.split('@')[0]}</div>}
          <div className="hero-main-title">
            <div className="hero-icon-row">
              <span className="hero-icon">📸</span><span className="hero-arrow-icon">→</span><span className="hero-icon">✨</span>
            </div>
            <h1>Генератор описания товара по фото</h1>
          </div>
          ...
          <div className="hero-features">
            ...
            {!user && <div className="hero-feature"><Shield size={16} /><span>Без регистрации</span></div>}
          </div>
        ...
        <div className="card upload-card">
          ...
          <div className="style-selector">
            ...
            <div className="style-options">
              {STYLES.map(s => {
                const locked = !user && !s.free;
                return (
                  <button
                    key={s.id}
                    className={`style-option ${selectedStyle === s.id ? 'active' : ''} ${locked ? 'locked' : ''}`}
                    onClick={() => {
                  ...
                  >
                    ...
                    <span className="style-desc">{s.desc}{locked ? ' 🔒' : ''}{s.free ? ' ✨' : ''}</span>
                  </button>
                );
              })}
            </div>
